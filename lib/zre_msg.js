...
class ZreMsg {
   ...
   * @param {(String|Buffer)} [options.content] - Content of the message
   ...
   * @param {boolean} [options.fullDuplex=false] - Supports full-duplex
   */
  ...
  /**
   * Sends this ZreMsg with the given ZyreChannel.
   *
  ...
  async send(channel) {
    const msg = this.content
      ? [this.toBuffer(), Buffer.isBuffer(this.content) ? this.content : Buffer.from(this.content)]
      : this.toBuffer();
  ...
  toBuffer() {
    ...
    if (this.fullDuplex === true) bufArr.push(putNumber1(1));
    ...
    try {
      ...
      switch (getNumber1(buffer.slice(2))) {
        case HELLO: {
          ...
          const headers = getDictionary(buffer.slice(pointer));
          pointer += headers.pointer;
          const fullDuplex = (pointer < buffer.length) && getNumber1(buffer.slice(pointer)) === 1;
