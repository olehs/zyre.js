...
const ZyreChannel = require('./zyre_channel');
const ZreMsg = require('./zre_msg');
const ZHelper = require('./zhelper');

const debug = ZHelper.debug('zyre:zyre_peer');
...
class ZyrePeer extends EventEmitter {
   ...
   * @param {zeromq.Router} [options.socket] - zeromq.Router socket
  ...
  constructor({
    ...
    this._sequenceOut = 0;
    this._evasiveAt = 0;
    this.passive = typeof socket !== 'undefined';

    this._createHandler();

    this._channel = new ZyreChannel({ identity, originID, socket });
   ...
   */
  async connect() {
    await this._channel.connect(this._endpoint);
  }
  ...
  async disconnect() {
    ...
    await this._channel.disconnect(this._endpoint);
  ...
  async send(msg) {
    ...
    const cmd = await msg.send(this._channel);
  ...
  update({
    ...
    if (!this._setSequence(sequence)) {
      if (this.passive) {
        this._sequenceOut = 0; // force disconnect
  ...
  _setAddress(address, mailbox) {
    if (typeof address !== 'undefined' && typeof mailbox !== 'undefined') {
      ...
      if (!this._channel.connected) this._endpoint = `tcp://${address}:${mailbox}`;
    }
  ...
  _setEndpoint(endpoint) {
    if (typeof endpoint !== 'undefined' && !this._channel.connected) this._endpoint = endpoint;
  }
  ...
  _createHandler() {
     ...
     *
     * @protected
     * @fires ZyrePeer#message
     */
    this._messageHandler = (id, msg, frame) => {
      /**
       * @event ZyrePeer#message
       ...
       */
      this.emit('message', id, msg, frame);
    };
  }
}
