...
class Zyre extends EventEmitter {
   ...
   * @param {number} [options.eport=65535] - Max port number to stop searching
  ...
  constructor({
    ...
    if (typeof eport === 'number') this._eport = eport; else this.eport = 65535;
    if (typeof bport === 'number') this._bport = bport;
    if (typeof binterval === 'number') this._binterval = binterval;
    this._fullDuplex = fullDuplex === true;
  ...
  async start(callback) {
    ...
    const mailbox = await ZHelper.getFreePort(this._ifaceData.address, this._port, this._eport);
    const node = new ZyreNode({
      identity: this._identity,
      name: this._name,
    ...
    node.on('hello', this._connectHandler);
    node.on('whisper', this._whisperHandler);
    node.on('shout', this._shoutHandler);
    node.on('join', this._joinHandler);
    node.on('leave', this._leaveHandler);
    ...
    await this._zyreNode.startListening();
    await this._zBeacon.start();
  ...
  async stop(callback) {
    ...
    await this._zyrePeers.disconnectAll();
    await this._zBeacon.stop();
    await this._zyreNode.stopListening();
   ...
   * @return {Promise}
   */
  async whisper(identity, message) {
    if (this._zyrePeers.exists(identity)) {
      const peer = this._zyrePeers.getPeer(identity);
      await peer.send(new ZreMsg(ZreMsg.WHISPER, {
        content: message,
      }));
    }
  }
   ...
   * @return {Promise}
   */
  async shout(group, message) {
    if (this._zyreGroups.exists(group)) {
      const theGroup = this._zyreGroups.getGroup(group);
      await theGroup.send(new ZreMsg(ZreMsg.SHOUT, {
        group,
        content: message,
      }));
    }
   ...
   */
  async join(group) {
    await this._zyreNode.join(group);
  }
   ...
   */
  async leave(group) {
    await this._zyreNode.leave(group);
  }
...
module.exports = Zyre;
module.exports.debug = ZHelper.debug;
